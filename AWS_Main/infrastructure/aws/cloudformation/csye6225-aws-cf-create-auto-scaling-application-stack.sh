...
#echo $lambda
#dn="*.$domainName"
SSLArn=$(aws acm list-certificates --query "CertificateSummaryList[?DomainName=='$domainName'].CertificateArn" --output text)
echo "SSLArn: $SSLArn"
 ...
 --parameters ParameterKey=stackname,ParameterValue=$1 ParameterKey=subnet1ID,ParameterValue=$2-subnet1ID ParameterKey=subnet2ID,ParameterValue=$2-subnet2ID ParameterKey=subnet3ID,ParameterValue=$2-subnet3ID ParameterKey=publicsubnet1ID,ParameterValue=$2-publicsubnet1ID ParameterKey=publicsubnet2ID,ParameterValue=$2-publicsubnet2ID ParameterKey=publicsubnet3ID,ParameterValue=$2-publicsubnet3ID ParameterKey=vpcID,ParameterValue=$2-vpcID ParameterKey=amiID,ParameterValue=$amiid ParameterKey=s3Name,ParameterValue=$s3name ParameterKey=RDSUser,ParameterValue=$rdsuser ParameterKey=RDSPassword,ParameterValue=$rdspwd ParameterKey=DeployS3,ParameterValue=$deploys3 ParameterKey=AWSID,ParameterValue=$usrid ParameterKey=DomainName,ParameterValue=$domainName ParameterKey=SSLArn,ParameterValue=$SSLArn ParameterKey=ODomainName,ParameterValue=$domain && aws cloudformation wait stack-create-complete --stack-name $1 || aws cloudformation describe-stack-events --stack-name $1 | grep -B2 -A8 CREATE_FAILED
	...
	if [ $? -eq 0 ];then
		...
		if [ "$status" == "CREATE_COMPLETE" ];then
			...
			echo ""
			echo "Please check your endpoint to see if tomcat is up and running, then perform the first deployment."
